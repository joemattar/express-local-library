...
// The first thing to note is that instead of being a single middleware function (with arguments (req, res, next))
...
exports.book_create_post = [
  ...
  // Is the req.body.genre not an array should the number of genres be 1 or less ??
  (req, res, next) => {
  ...
  (req, res, next) => {
    ...
    // Data from form is valid. Save book. Creates New Object in DB
    book.save((err) => {
      if (err) {
        return next(err);
      }
...
exports.book_delete_get = (req, res, error) => {
  async.parallel(
    {
      book(callback) {
